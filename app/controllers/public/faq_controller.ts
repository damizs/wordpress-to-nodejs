...
function buildDefaultPublicFaqs(siteSettings: Record<string, string | null>) {
  const orgName = camara.nome
  const address = siteSettings.footer_address || `sede da ${orgName}`
  const hours = siteSettings.footer_hours || 'conforme horário de atendimento publicado no portal'
  const phone = siteSettings.footer_phone ? ` pelo telefone ${siteSettings.footer_phone}` : ''
  const email = siteSettings.footer_email ? ` ou pelo e-mail ${siteSettings.footer_email}` : ''

  return [
    {
      question: 'Qual o horário de funcionamento da Câmara?',
      answer: `${orgName} atende ao público em ${address}, ${hours}. Também é possível entrar em contato${phone}${email} pelos canais oficiais publicados no portal.`,
...
export default class FaqController {
  async index({ inertia }: HttpContext) {
    ...
    for (const fallback of buildDefaultPublicFaqs(siteSettings)) {
