...
export default class LegislativeActivitiesController {
  async index({ inertia, request }: HttpContext) {
    ...
    if (search) query = query.where((q) => {
      q.whereILike('summary', `%${search}%`).orWhereILike('number', `%${search}%`)
    })

    const activities = await query.paginate(page, 20)

    // Get distinct types and years for filters
    const types = await LegislativeActivity.query()
      .distinct('type')
      .orderBy('type')
    const years = await LegislativeActivity.query()
      .distinct('year')
      .orderBy('year', 'desc')
  ...
  async store({ request, response, session }: HttpContext) {
    const data = request.only([
      'type', 'number', 'year', 'summary', 'content', 'status', 'author', 'file_url', 'session_date',
    ])
    data.year = parseInt(data.year)

    await LegislativeActivity.create(data)
