...
const BLOCK_DEFS: { type: BlockType; label: string; icon: LucideIcon; make: () => EditorBlock }[] =
  [
    ...
    { type: 'columns', label: 'Colunas', icon: Columns3, make: () => ({ type: 'columns', layout: '1-1', columns: [[], []] }) },
  ]
...
// Lista de blocos aninhada (dentro de uma coluna). Reusa BlockFields para
// editar cada bloco; não permite aninhar 'colunas' dentro de colunas.
function NestedBlockList({
  ...
  const add = (type: BlockType) => onChange([...blocks, blockDef(type).make()])
  ...
  return (
    <div className="space-y-2">
      {blocks.map((block, i) => {
        ...
        return (
          <div key={i} className="rounded-lg border border-border bg-card">
            <div className="flex items-center justify-between gap-1 px-2 py-1.5 border-b border-border/60">
              <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-navy">
                <DefIcon className="w-3.5 h-3.5" />
              ...
              <div className="flex items-center gap-0.5">
                ...
                <IconButton
                  ...
                  className={i === blocks.length - 1 ? 'opacity-30 pointer-events-none' : ''}
                  onClick={() => move(i, 1)}
                >
                  <ChevronDown className="w-3.5 h-3.5" />
                </IconButton>
                <IconButton tone="delete" title="Remover" onClick={() => remove(i)}>
      ...
      <div className="flex flex-wrap gap-1">
        {BLOCK_DEFS.filter((d) => d.type !== 'columns').map((d) => (
          <button
            type="button"
            key={d.type}
            onClick={() => add(d.type)}
            className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded border border-border bg-muted hover:bg-muted/70 text-foreground"
          >
...
function ColumnsEditor({ block, onChange }: BlockEditorProps) {
  const layout: string = block.layout || '1-1'
  const columns: EditorBlock[][] = Array.isArray(block.columns) ? block.columns : [[], []]
  const LAYOUTS = [
    { key: '1-1', label: '50 / 50', n: 2 },
    { key: '1-2', label: '1/3 + 2/3', n: 2 },
    { key: '2-1', label: '2/3 + 1/3', n: 2 },
    { key: '1-1-1', label: '3 colunas', n: 3 },
  ]
  const applyLayout = (key: string) => {
    const n = LAYOUTS.find((l) => l.key === key)?.n || 2
    const cols = [...columns]
    while (cols.length < n) cols.push([])
    onChange({ layout: key, columns: cols.slice(0, n) })
  }
  const setColumn = (ci: number, list: EditorBlock[]) =>
    onChange({ columns: columns.map((c, i) => (i === ci ? list : c)) })
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {columns.map((col, ci) => (
          <div key={ci} className="rounded-lg border border-dashed border-border/70 p-2 bg-muted/20">
            <div className="text-[11px] font-semibold text-muted-foreground mb-1.5 uppercase tracking-wide">
              Coluna {ci + 1}
            </div>
            <NestedBlockList
              blocks={Array.isArray(col) ? col : []}
              onChange={(list) => setColumn(ci, list)}
            />
          </div>
        ))}
      </div>
