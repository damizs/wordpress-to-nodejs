...
export default function NewsIndex({ news }: Props) {
  ...
  const getImage = (item: NewsItem) => item.featured_image || item.coverImageUrl || item.cover_image_url;
  const getDate = (item: NewsItem) => item.published_at || item.publishedAt || new Date().toISOString();
