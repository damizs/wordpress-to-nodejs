  ...
  return (
    <li ref={ref} className="relative" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <a
        ...
        className={`flex items-center gap-1 px-4 py-2.5 text-sm font-medium tracking-wide rounded-xl hover:bg-primary-foreground/10 transition-all duration-300 cursor-pointer ${open ? 'bg-primary-foreground/10' : ''}`}
      >
        {item.label}
        <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
      </a>
      <div
        className={`absolute top-full left-0 pt-2 z-50 transition-all duration-200 ${open ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-1'}`}
      >
        <div className="w-56 bg-white rounded-xl shadow-xl border border-gray-100 py-2">
          {item.children.map((child, i) => (
            <a
              key={i}
              href={child.href}
              className="block px-4 py-2.5 text-sm text-gray-700 hover:bg-amber-400 hover:text-navy-dark transition-colors duration-150"
            >
              {child.label}
            </a>
          ))}
    ...
    <header className="relative bg-gradient-hero text-primary-foreground overflow-visible">
      ...
      <div className="relative container mx-auto px-4 py-8">
        <a href="/" className="flex items-center justify-center gap-5 mb-8 animate-fade-in no-underline text-inherit">
          {logoUrl ? (
            <img src={logoUrl} alt={title} className="h-24 md:h-32 object-contain drop-shadow-lg" />
          ) : (
            <>
